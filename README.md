...
- 🏷️ **刮削**:多源降级(豆瓣网页抓取 → Google Books → Open Library → 手动),刮削过程可视化(逐步展示请求/命中/失败原因),管理员可配置豆瓣 Cookie 提高成功率
...
在书籍详情页点「刮削」可从豆瓣 / Google Books / Open Library 抓取标题、作者、出版信息、简介与封面。对话框内会**实时展示刮削过程日志**(请求了哪个源、命中/失败、失败原因),方便定位问题。
